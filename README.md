...
- **한국어 UI 지원**: Clerk 컴포넌트 전체 한국어 번역
  - 로그인/회원가입 페이지 한국어
  - 에러 메시지 한국어
  - 커스텀 번역 지원

### 🗄️ 데이터베이스 통합
- **Supabase 공식 문서 모범 사례 적용**: `@supabase/ssr` 패키지 사용
- Clerk 토큰 기반 Supabase 인증 (JWT 템플릿 불필요)
- Cookie-based 세션 관리
- 환경별 Supabase 클라이언트 분리:
  - Client Component용 (`useClerkSupabaseClient`) - `createBrowserClient` 사용
  - Server Component용 (`createClient`) - `createServerClient` 사용
  - 관리자 권한용 (`getServiceRoleClient`)
- SQL 마이그레이션 시스템
- Supabase 공식 문서 예시 페이지 포함 (`/instruments`)
...
2. 프로젝트 선택 → **Settings** → **Authentication** → **Providers**로 이동
...
통합이 완료되면 Clerk 세션 토큰이 Supabase에서 자동으로 검증됩니다. 자세한 내용은 [Clerk Supabase 통합 가이드](./docs/clerk-supabase-integration.md)를 참고하세요.
...
프로젝트 루트에 `.env.example` 파일이 있습니다. 이를 복사하여 `.env` 파일을 생성하세요:

```bash
cp .env.example .env
```

그 다음 `.env` 파일을 열어 실제 값으로 교체하세요.
...
**테스트 페이지:**
- `/instruments`: Supabase 공식 문서 예시 기반 테스트 페이지
...
- `app/layout.tsx`: `ClerkProvider`에 `clerkLocalization` 적용

**커스터마이징**:
- `lib/clerk/localization.ts` 파일을 수정하여 특정 텍스트를 브랜드에 맞게 변경할 수 있습니다
- 에러 메시지도 커스터마이징 가능합니다
